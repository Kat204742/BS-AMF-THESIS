...
funcpar <- function(a, b, d, SDR, hypotest) 
{
...
funcnonpar <- function(a, b, d, SDR, hypotest)
{
  shape2=(2/(2/sqrt(d)-0.5))^2
  e=SDR*sqrt(shape2/d)
      ...
      for (SDR in c(1,1.1,1.25,1.5,2)) 
      {
        for (hypotest in c("ttest","welch"))
        {
          ...
          if(hypotest=="ttest")
          {
            ...
            ttest.column.labelspar <- cbind(ttest.column.labelspar,paste("gettest",a,'.',b,' sk',sk,' sdr',SDR,sep=''))
          }
          if(hypotest=="welch")
          {
            ...
            welch.column.labelspar <- cbind(welch.column.labelspar,paste("gewelch",a,'.',b,' sk',sk,' sdr',SDR,sep=''))
          }
          counterleft <-0
          counterright <-0
          testvector <- NULL
      ...
      for (SDR in c(1.1)) 
      {
        for (hypotest in c("boots","perms")) 
        {
          ...
          if(hypotest=="boots")
          {
            ...
            boots.column.labelspar <- cbind(boots.column.labelspar,paste("geboots",a,'.',b,' sk',sk,' sdr',SDR,sep=''))
          ...
          if(hypotest=="perms")
          {
            ...
            perms.column.labelspar <- cbind(perms.column.labelspar,paste("geperms",a,'.',b,' sk',sk,' sdr',SDR,sep=''))
            perms.count10kmatrix <- cbind(perms.count10kmatrix,count10k)
          }
          print(paste("left ",hypotest,a,'.',b,' sk',sk,' sdr',SDR,": ",counterleft,sep=''))
          print(paste("right ",hypotest,a,'.',b,' sk',sk,' sdr',SDR,": ",counterright,sep=''))
          counterleft <- 0
          counterright <- 0
          testvector <- NULL
          pvalleftvector <- NULL
