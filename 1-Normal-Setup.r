...
funcpar <- function(a, b, d, hypotest) 
{
  normal.1 <- rnorm(a, mean=5, sd=d) 
  ...
  n1=a
  n2=b
  return(list(x.stat=x.stat, x=x, normal.1=normal.1, normal.2=normal.2, n1=n1, n2=n2))
}

funcnonpar <- function(a, b, d, hypotest) 
{
  normal.1 <- rnorm(a, mean=5, sd=d) 
  normal.2 <- rnorm(b, mean=0, sd=4) 
  ...
  x <- (t.test(normal.1, normal.2, var.equal = FALSE))
  tobs=x$statistic-5/sqrt(var(normal.1)/a+var(normal.2)/b)
  ...
  n1=a
  n2=b
  return(list(pvalleft=pvalleft, pvalright=pvalright, tobs=tobs, normal.1=normal.1, normal.2=normal.2, n1=n1, n2=n2))
}
    ...
    for (d in c(4,4.4,5,6,8)) 
    {
      for (hypotest in c("ttest","welch"))
      {
        ...
        if(hypotest=="ttest")
        {
          ...
          ttest.column.labelspar <- cbind(ttest.column.labelspar,paste("ttest",a,'.',b,' sdr',d/4,sep=''))
        }
        if(hypotest=="welch")
        {
          ...
          welch.column.labelspar <- cbind(welch.column.labelspar,paste("welch",a,'.',b,' sdr',d/4,sep=''))
        }
        counterleft <-0
        counterright <-0
        testvector <- NULL
    ...
    for (d in c(4)) 
    {
      for (hypotest in c("boots","perms"))
      {
        ...
        if(hypotest=="boots")
        {
          ...
          boots.column.labelspar <- cbind(boots.column.labelspar,paste("boots",a,'.',b,' sdr',d/4,sep=''))
        ...
        if(hypotest=="perms")
        {
          ...
          perms.column.labelspar <- cbind(perms.column.labelspar,paste("perms",a,'.',b,' sdr',d/4,sep=''))
          perms.count10kmatrix <- cbind(perms.count10kmatrix,count10k)
        }
        print(paste("left ",hypotest,a,'.',b,' sdr',d/4,": ",counterleft,sep=''))
        print(paste("right ",hypotest,a,'.',b,' sdr',d/4,": ",counterright,sep=''))
        counterleft <- 0
        counterright <- 0
        testvector <- NULL
        pvalleftvector <- NULL
